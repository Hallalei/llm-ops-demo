/**
 * 停止任务 API
 * POST /api/tasks/[taskName]/stop
 */

import { apiError, apiSuccess } from "@/lib/api";
import { withRoles } from "@/lib/auth/api-guards";
import { stopTask, type TaskName } from "@/lib/task-manager";
...
export const POST = withRoles(
  ...
  async (
    ...
    try {
      ...
      const success = await stopTask(taskName as TaskName);

      if (success) {
        return apiSuccess({ message: `任务 ${taskName} 已停止` });
      } else {
        return apiError("任务停止失败，可能未注册", 400);
      }
    } catch (error) {
      return apiError(
        "停止任务失败",
