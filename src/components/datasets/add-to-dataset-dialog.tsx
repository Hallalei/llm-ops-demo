...
export function AddToDatasetDialog({
  ...
  const handleSubmit = async () => {
    ...
    try {
      // Add to all selected datasets
      await Promise.all(
        Array.from(selectedIds).map((datasetId) =>
          fetch(`/api/datasets/${datasetId}/items`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
              source,
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          <DialogTitle>添加到数据集</DialogTitle>
          <DialogDescription>
            将此对话添加到一个或多个数据集中
          </DialogDescription>
        </DialogHeader>
          ...
          <div className="space-y-2 rounded-md border bg-muted/50 p-3">
            <div className="flex items-center gap-2">
              <Badge variant="outline">
                {source === "session" ? "会话" : "Trace"}
              </Badge>
              {source === "session" && preview.contextCount !== undefined && (
                <Badge variant="secondary">
                  {preview.contextCount} 条上下文
                </Badge>
              )}
            </div>
